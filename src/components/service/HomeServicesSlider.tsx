...
// All 11 Services Data — displayed in two infinite marquee rows
const sliderServicesRow1 = [
...
function MarqueeRow({ services, reverse = false }: { services: typeof sliderServicesRow1; reverse?: boolean }) {
  return (
    <div className={`tp-marquee-row${reverse ? ' tp-marquee-row--reverse' : ''}`}>
      {/* Track containing 3 copies for seamless loop on wide screens */}
...
export default function HomeServicesSlider() {
  return (
    <section className="tp-home-services-slider" aria-label="Our services">
      ...
      {/* Row 1 — scrolls left */}
      <MarqueeRow services={sliderServicesRow1} />

      {/* Row 2 — scrolls right (reversed) */}
      <MarqueeRow services={sliderServicesRow2} reverse />
    </section>
  );
}
