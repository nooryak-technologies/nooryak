  ...
  useEffect(() => {
    ...
    class Slider {
      ...
      constructor(element: HTMLElement, reverseDirection = false) {
        ...
        // ✅ Typed querySelectorAll + Array.from
        this.items = Array.from(
          element.querySelectorAll<HTMLElement>(".tp-portfolio-slider__item")
        );
        ...
        // ✅ Safe add current class
        if (this.items.length > 0) {
          this.items[this._current].classList.add("current");
        }

        gsap.set(
          [...this.items, ...this.itemsInner.filter(Boolean) as HTMLElement[]],
      ...
      navigate(direction: number) {
        ...
        if (!currentItem || !upcomingItem || !currentInner || !upcomingInner) {
          this.isAnimating = false; // prevent lock
          return;
        }
        ...
        gsap
          ...
          .to(currentItem, {
            xPercent: this.reverseDirection
              ...
              : -direction * 100,
            onComplete: () => {
              if (currentItem) gsap.set(currentItem, { opacity: 0 });
            },
          })
          .to(
            currentInner,
    ...
    const navigate = (dir: SliderDirection) => {
      sliderBG[dir]();
      sliderFG[dir]();
    };
